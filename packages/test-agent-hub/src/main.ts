...
function printScenarioList(tags: string[]): void {
  ...
  console.log(`${BOLD}Available scenarios${RESET} (${filtered.length}/${all.length}):`);
  console.log();

  for (const s of filtered) {
    const tagStr = s.tags.length > 0 ? ` ${DIM}[${s.tags.join(', ')}]${RESET}` : '';
...
async function main(): Promise<void> {
  const argv = await yargs
    ...
    .option('list', {
      type: 'boolean',
      default: false,
      describe: 'List available scenarios and exit (does not require --agentId)',
      group: 'Output:',
    })
    .option('verbose', {
      type: 'boolean',
  ...
  if (argv.config) {
    ...
    if (!fs.existsSync(configPath)) {
      console.error(`${RED}Error:${RESET} Config file not found: ${configPath}`);
      process.exit(2);
    }
    try {
      ...
      console.error(`${RED}Error:${RESET} Failed to parse config file: ${(err as Error).message}`);
      process.exit(2);
    }
  }
  ...
  const nestLogLevels: ('error' | 'warn' | 'log' | 'debug' | 'verbose')[] = argv.verbose
